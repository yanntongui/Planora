...
const SignupView: React.FC<SignupViewProps> = ({ onSwitch }) => {
    ...
    const { login, signupWithPassword, isLoading, error } = useAuth();
    const { t } = useLanguage();
    const [isSent, setIsSent] = useState(false);

    // Using the same login method as Supabase 'signInWithOtp' handles both sign up and sign in for magic links
    ...
    return (
        <AuthLayout
            ...
            subtitle={isSent ? 'Vérifiez votre email' : t('auth.signupSubtitle')}
        >
            {isSent ? (
                <div className="space-y-6">
                    <div className="bg-green-100 dark:bg-green-900/30 p-4 rounded-lg text-green-800 dark:text-green-200 text-sm">
                        Un lien de connexion a été envoyé à <strong>{email}</strong>. Cliquez dessus pour activer votre compte.
                    </div>
                    <button
                ...
                <>
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="flex items-center gap-2 mb-2">
                            <input
                                type="checkbox"
                                id="usePasswordSignup"
                                checked={usePassword}
                                onChange={(e) => setUsePassword(e.target.checked)}
                                className="rounded border-zinc-300 dark:border-zinc-700 text-purple-600 focus:ring-purple-500"
                            />
                            <label htmlFor="usePasswordSignup" className="text-sm text-zinc-600 dark:text-zinc-400 cursor-pointer">
                        ...
                        >
                            {isLoading ? <LoaderIcon className="w-5 h-5 animate-spin" /> : <>{t('auth.getStartedButton') || t('auth.signUpButton')} <ArrowRightIcon className="w-4 h-4" /></>}
                        </button>
                    </form>
